...
class Game:
    def __init__(self):
        ...
        self.alien_lasers = AlienLasers(game=self)
        self.ship.set_alien_fleet(self.alien_fleet)
        self.ship.set_lasers(self.lasers)
        self.bg = pg.image.load("images/bg3.png")
    ...
    def update(self):
        ...
        alien_list = self.alien_fleet.fleet.sprites()
        if pg.time.get_ticks() % 360 == 0:
            rand_alien = random.randint(0, len(alien_list)-1)
            self.alien_lasers.fire(alien=alien_list[rand_alien])
