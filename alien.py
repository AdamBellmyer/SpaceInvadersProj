...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'images/explosion_{n}.png') for n in range(4)]
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/alien__{m}{n}.png'), 0, .5) for n in range(2)] for m in range(3)]
    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'images/spaceship2_{n}.png'), 0, 0.5) for n in range(2)]
    alien_images.append(ufo_imgs)
    alien_points = [75, 30, 15, 150]
