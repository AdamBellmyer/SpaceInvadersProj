...
class Ship(Sprite):
  exploding_images = [pg.image.load(f'images/shipExplosion_{n}.png') for n in range(9)]
  images = [pg.image.load(f'images/customShipTest2.png') for n in range(1)]
  ...
  def update(self):
    ...
    if self.frames % 30 == 0 and self.firing:
      self.lasers.fire()
    self.frames += 1
    if pg.sprite.spritecollideany(self, self.game.alien_lasers.alien_lasers):
      self.hit()
